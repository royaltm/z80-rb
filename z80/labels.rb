...
module Z80
	...
	#
	#  ===Label names
	#  Label name may be any name that is a valid ruby method name and is not a singleton method name of your *program*.
	#  It excludes:
	#  * ruby statement
	...
	#  It is also wise to add numeric sufixes to label names:
	#    loop1 label
	#
	...
	class Label
		...
		# This methos is being used during program compilation.
		...
		def reinitialize(address, type = 1, reloc = nil, members = nil)
			...
			@sizelabel.reinitialize(@size, 1) if @sizelabel
			self
		end
		def to_alloc
			Alloc.new(self)
	...
	class Alloc
		...
		def to_str
			@label.to_name.to_s + @index.map {|i|
				if String === i
					'.' + i
				else
					"*#{i}"
