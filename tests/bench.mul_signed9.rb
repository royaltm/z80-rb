...
program = ZXLib::Basic.parse_source <<-EOC
  ...
  20 PRINT "See results on ZX Printer": OPEN #2,"#{channel}": PRINT "SinCos: T-States size: #{routine_size}"
  30 LET sum=0: LET max=0: LET maxi=-1: LET min=1e+38: LET mini=-1: LET v=0
  50 FOR m=-256 TO 255: FOR k=-256 TO 255
 100 RANDOMIZE FN z(k,m): LET frames=FN b(#{test_mul_signed9},counter)
