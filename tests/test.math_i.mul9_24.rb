...
class MTestFactory
    ...
    module Macros
        ...
        def make_tests(tt=de, optimize=:time, signed:false)
            ...
            # a|hl = kk * tt
            multiply    mul8_24(th, tl, kl, t:sgn, tt:tt, clrahl: true, k_int24:signed, mbit9_carry:true, optimize:optimize)
                        ret

            multiply_a  mul8_24a(th, tl, kl, t:sgn, tt:tt, k_int24:signed, mbit9_carry:true, optimize:optimize)
                        anda a # clear CF
                        ret

            # a|hl += kk * tt
            multadd     mul8_24(th, tl, kl, t:sgn, tt:tt, clrahl:false, k_int24:signed, mbit9_carry:true, optimize:optimize)
                        ret
        end
    end
end
...
class MTest1
    ...
    SIGNED = false
    NAME = "test.math_i.mul9_24.unsigned_16.time"
...
class MTest2
    ...
    SIGNED = false
    NAME = "test.math_i.mul9_24.unsigned_16.size"
...
class MTest3
    ...
    SIGNED = true
    NAME = "test.math_i.mul9_24.signed_24.time"
...
class MTest4
    ...
    SIGNED = true
    NAME = "test.math_i.mul9_24.signed_24.size"
...
[MTest1, MTest2, MTest3, MTest4].each do |mtest_klass|
    ...
    # puts mtest.debug
    source = <<-END
       1 DEF FN m(a,b)=USR #{mtest[:test_mul24_9]}: DEF FN n(a,b)=USR #{mtest[:test_mul24_9a]}: DEF FN a(a,b,c)=USR #{mtest[:test_mul]}
      10 RANDOMIZE
      20 FOR b=0 TO 511
    END
    if mtest_klass::SIGNED
        source << <<-END
         LET a=INT (RND*32833)-16416: PRINT AT 0,0;a;"*";b;"=";: LET r=FN m(a,b)
        ...
        source << <<-END
         LET a=INT (RND*32833): PRINT AT 0,0;a;"*";b;"=";: LET r=FN m(a,b)
         IF r<>a*b THEN GO TO 1000
         LET r=FN n(a,b): IF r<>a*b THEN GO TO 1100
         PRINT r;"        "
